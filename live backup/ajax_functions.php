...
if ($action == 'get_ebay_details') {
    try {
        ...
        if (isset($data->itemSummaries)) {
            ...
            $summeries_from_first_call = $data->itemSummaries;
        }

        $next_url = $data->next ?? null;
        $inner_new_call = false;

        do {
            $data_summeries_to_write = [];

            if ($next_url) {
                if ($inner_new_call == true) {
                    ...
                    if (isset($next_data->itemSummaries)) {
                        $all_item_summaries = array_merge($all_item_summaries, $next_data->itemSummaries);
                        $data_summeries_to_write = array_merge($summeries_from_first_call, $next_data->itemSummaries);
            ...
            if (isset($data_summeries_to_write) && is_array($data_summeries_to_write)) {
                ...
                foreach ($data_summeries_to_write as $result) {
                    ...
                    <td>';
                        ...
                        fputcsv($fp, array("Product", $title, "P", "", "", "", "", "Right", "", "0", $price, "0", "0", "0", "Y", "6 Month Replacement Warranty", "7", "0", "0", "0", "Y", "Y", "Usually Ships in 1-2 Business Days", "none", "0", "0", $category_name, $large_image, "", $img_2, "Y", $img_3, "", "", "", "unique_string", $itemId_onEbay, $condition_name, "Y", "0", "Default Tax Class", "", "N", "", "", "", "", "", "", "", "", "", "", "", "", "N", "", "", $itemId_onEbay));
                    }
                }
            }
